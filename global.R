...
init = function(){
  ...
  dir_scenario <<- file.path(y$default_branch, y$default_scenario) # './data/ohi-global/eez2015'
}
init()
...
if (!file.exists(rdata)){
  ...
  if (!dir.exists(dir_scenario)) stop(sprintf('Although app.yml default_branch folder "%s" exists, \nit does not contain subfolder "%s" as specified by default_scenario in app.yml. \nRecommend deleting local default_branch folder and rerun Shiny app to clone original, or update app.yml to have correct default_scenario.', y$default_branch, y$default_scenario))
